...
enum ColorDataListsDemo {
  ...
  static void main() throws IOException {
    ...
    for (int count = size - spa; count < large.length(); count += size)
      for (int i = 0; i < spa; ++i)
        large.set(_ -> RealScalar.ZERO, count + i, Tensor.ALL, Tensor.ALL);
    ...
    Export.of(HomeDirectory.Pictures.resolve(ColorDataLists.class.getSimpleName().toLowerCase() + ".png"), large);
  }
}
